...
class Trainval(object):
    ...
    def train(self):
        ...
        for i, data in enumerate(tbar):
            ...
            cd_loss, seg_loss = self.model(data['img1'].cuda(),
                                           data['img2'].cuda(), data['cd_label'].cuda(),
            ...
            tbar.set_description("Loss: %.3f, CD: %.3f, Seg: %.3f, LR: %.6f" %
                                 (_loss / (i + 1), _cd_loss / (i + 1), _seg_loss / (i + 1),
        ...
        with torch.no_grad():
            for i, _data in enumerate(tbar):
                ...
                tbar.set_description(
                    "Score: %.2f, IOU: %.2f, SeK: %.2f" % (score * 100.0, miou * 100.0, sek * 100.0))
