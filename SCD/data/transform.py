...
class Transforms(object):
    def __call__(self, _data):
        img1, img2, label1, label2, cd_label = _data['img1'], _data['img2'], _data['label1'], _data['label2'], _data['cd_label']
        ...
        if random.random() < 0.5:
            ...
            label1 = TF.resized_crop(label1, i, j, h, w, size=(256, 256), interpolation=InterpolationMode.NEAREST)
            label2 = TF.resized_crop(label2, i, j, h, w, size=(256, 256), interpolation=InterpolationMode.NEAREST)
            cd_label = TF.resized_crop(cd_label, i, j, h, w, size=(256, 256), interpolation=InterpolationMode.NEAREST)

        return {'img1': img1, 'img2': img2, 'label1': label1, 'label2': label2, 'cd_label': cd_label}
