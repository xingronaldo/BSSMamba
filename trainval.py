...
class Trainval(object):
    ...
    def train(self):
        ...
        for i, data in enumerate(tbar):
            ...
            focal, dice, ds_loss, boundary_coh_loss, boundary_cls_loss = self.model(data['img1'].cuda(),
                                                                                     data['img2'].cuda(),
                                                                                     data['cd_label'].cuda(),
                                                                                     data['boundary_mask'].cuda(),
                                                                                     data['boundary_label'].cuda())
            loss = focal * 0.5 + dice + ds_loss * 0.5 + boundary_coh_loss * 0.5 + boundary_cls_loss * 0.5
            ...
            _ds_loss += ds_loss.item()
            _coh_loss += boundary_coh_loss.item()
            _cls_loss += boundary_cls_loss.item()
            del loss

            tbar.set_description("Loss: %.3f, Focal: %.3f, Dice: %.3f, DS: %.3f, Coh: %.3f, Cls: %.3f, LR: %.6f" %
                                 (_loss / (i + 1), _focal_loss / (i + 1), _dice_loss / (i + 1), _ds_loss / (i + 1),
                                  _coh_loss / (i + 1), _cls_loss / (i + 1), self.optimizer.param_groups[0]['lr']))
